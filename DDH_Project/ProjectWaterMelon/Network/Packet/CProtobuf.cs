...
namespace ProjectWaterMelon.Network.Packet
{
    ...
    {
        public static byte[] BuildPacket<T>(T data, Protocol.PacketId type) where T: class
        {
            if (data == null)
                return null;
            ...
            System.Buffer.BlockCopy(BitConverter.GetBytes((uint)type), 0, result, MAX_PACKET_HEADER_SIZE, MAX_PACKET_TYPE_SIZE);
        ...
        public static byte[] ProtobufSerialize<T>(T data) where T : class
        {
        ...
        public static byte[] BinaryFtSerialize<T>(T data) where T : class
        {
            if (data == null)
                return null;
            ...
            try
            {
                using (MemoryStream lMemoryStream = new MemoryStream())
                {
                    ...
                    bf.Serialize(lMemoryStream, data);
                    return lMemoryStream.ToArray();
                }       
            }
            catch (Exception ex)
            {
                CLog4Net.LogError($"Exception in CProtobuf.BinaryftSerialize - {ex.Message} - {ex.StackTrace}");
                return null;
            }
        }
